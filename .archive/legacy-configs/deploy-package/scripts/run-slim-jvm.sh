#!/bin/sh
# Aurigraph V12 - Multi-Slim Node Runner (JVM Mode)
# Runs 5 slim nodes in a single container using Quarkus JVM
...
# Optimized JVM settings for low memory (targeting ~150MB per node)
JAVA_OPTS="${JAVA_OPTS:--Xms32m -Xmx150m -XX:+UseG1GC -XX:MaxGCPauseMillis=100 -XX:+UseStringDeduplication -XX:InitiatingHeapOccupancyPercent=45 -XX:G1HeapRegionSize=1m -XX:MaxMetaspaceSize=64m -Xss256k}"

echo "=== Aurigraph V12 Slim Node Cluster (JVM Mode) ==="
...
# Wait for initial startup (5 nodes x 3s stagger = 15s + 18s startup)
