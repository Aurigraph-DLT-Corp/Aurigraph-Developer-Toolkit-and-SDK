...
class AssetRegistrationService extends events_1.EventEmitter {
    ...
    async processRequest(requestId) {
        ...
        if (!request)
            return;
        const startTime = perf_hooks_1.performance.now( /* @ts-ignore */);
        ...
        try {
            ...
            // Update metrics
            const processingTime = perf_hooks_1.performance.now( /* @ts-ignore */) - startTime;
    ...
    async performVerification(requirement, request) {
        ...
        return {
            ...
            duration: perf_hooks_1.performance.now( /* @ts-ignore */) - startTime,
            timestamp: new Date()
        };
    }
    ...
    updateMetrics() {
        ...
        const completedLastHour = this.performanceHistory.filter(m => Date.now( /* @ts-ignore */) - new Date().getTime() < 3600000).length;
    ...
    // Utility methods
    generateRequestId() {
        return `REQ-${Date.now( /* @ts-ignore */)}-${(0, crypto_1.randomBytes)(4).toString('hex').toUpperCase()}`;
    }
    generateSlotId() {
        return `SLOT-${Date.now( /* @ts-ignore */)}-${(0, crypto_1.randomBytes)(4).toString('hex').toUpperCase()}`;
    }
    ...
    async submitRegistration(request) {
        const result = await this.submitRegistrationRequest(request.submitterId || 'system', request.assetData || request, request.documents || [], {
    ...
    async updateVerificationStatus(entityId, status) {
        // Find the request associated with this entity
        const request = Array.from(this.requests.values()).find(r => r.assetData.id === entityId || r.id === entityId);
        if (request) {
    ...
    getEstimatedProcessingTime(processingType) {
    ...
    async updateRegistrationPriority(requestId, priority) {
        const request = this.requests.get(requestId);
        if (!request)
            throw new Error(/* @ts-ignore */ 'Registration request not found');
    ...
    getPerformanceMetrics() {
        return {
            ...
            uptime: Date.now( /* @ts-ignore */) - this.startTime.getTime(),
            queueSizes: Object.fromEntries(Array.from(this.priorityQueues.entries()).map(([key, value]) => [key, value.length])),
            resourceUsage: this.metrics.resourceUsage
        };
    }
}
exports.AssetRegistrationService = AssetRegistrationService;
//# sourceMappingURL=AssetRegistrationService.js.map