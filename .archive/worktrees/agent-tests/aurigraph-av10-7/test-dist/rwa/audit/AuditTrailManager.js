...
class AuditTrailManager extends events_1.EventEmitter {
    constructor() {
        ...
        this.digitalSignatureKeys = new Map();
...
<AuditReport>
    ...
    getAuditMetrics() {
        ...
        // Enhanced metrics for AV10-21 verification systems
        const verificationEventsLast24h = this.auditChain.filter(e => e.timestamp > last24h && (e.eventType.includes('VERIFICATION') ||
            e.category === 'COMPLIANCE')).length;
        const dueDiligenceEventsLast24h = this.auditChain.filter(e => e.timestamp > last24h && e.eventType.includes('DUE_DILIGENCE')).length;
        const complianceAssessmentEvents = this.auditChain.filter(e => e.eventType.includes('COMPLIANCE_ASSESSMENT')).length;
        const escalationEventsLast7d = this.auditChain.filter(e => e.timestamp > last7d && (e.eventType.includes('ESCALAT') ||
            e.severity === 'CRITICAL')).length;
        // Calculate average verification time from verification events
        const verificationEvents = this.auditChain.filter(e => e.eventType === 'VERIFICATION_COMPLETED');
        const averageVerificationTime = verificationEvents.length > 0 ?
            verificationEvents.reduce((sum, e) => sum + (e.details?.processingTime || 0), 0) / verificationEvents.length : 0;
        const quantumSecurityEvents = this.auditChain.filter(e => e.details?.quantumSignature || e.eventType.includes('QUANTUM')).length;
        const multiSourceVerificationEvents = this.auditChain.filter(e => e.eventType === 'VERIFICATION_REQUEST_CREATED' &&
            (e.details?.sourcesCount || 0) > 1).length;
        return {
            totalEvents: this.auditChain.length,
            eventsLast24h,
            ...
            // Enhanced AV10-21 metrics
            verificationEventsLast24h,
            dueDiligenceEventsLast24h,
            complianceAssessmentEvents,
            escalationEventsLast7d,
            averageVerificationTime,
            quantumSecurityEvents,
            multiSourceVerificationEvents
        };
    }
    // Enhanced method for verification-specific audit logging
    async logVerificationEvent(verificationId, eventType, status, details, userId) {
        return await this.logEvent(`VERIFICATION_${eventType}`, 'COMPLIANCE', status === 'FAILED' ? 'HIGH' : 'MEDIUM', verificationId, 'VERIFICATION_PROCESS', status, {
            ...details,
            verificationId,
            auv10Enhancement: true,
            timestamp: Date.now()
        }, {
            nodeId: process.env.NODE_ID || 'verification-engine',
            verification: true,
            quantum: details.quantumSecurity || false
        }, userId);
    }
    // Enhanced method for due diligence audit logging
    async logDueDiligenceEvent(profileId, eventType, componentType, details = {}, userId) {
        const severity = details.escalationLevel && details.escalationLevel > 0 ? 'HIGH' : 'MEDIUM';
        return await this.logEvent(`DD_${eventType}`, 'COMPLIANCE', severity, profileId, 'DUE_DILIGENCE_PROFILE', eventType.split('_').pop() || 'ACTION', {
            ...details,
            profileId,
            componentType,
            av10DueDiligence: true,
            timestamp: Date.now()
        }, {
    ...
    // Enhanced method for compliance assessment audit logging
    async logComplianceEvent(assessmentId, eventType, jurisdiction, details = {}, userId) {
        const severity = details.criticalIssues && details.criticalIssues > 0 ? 'HIGH' : 'MEDIUM';
        return await this.logEvent(`COMPLIANCE_${eventType}`, 'REGULATORY', severity, assessmentId, 'COMPLIANCE_ASSESSMENT', eventType.split('_').pop() || 'ACTION', {
            ...details,
            assessmentId,
            jurisdiction,
            av10Compliance: true,
            ...
            nodeId: process.env.NODE_ID || 'legal-compliance-module',
            compliance: true,
            jurisdiction,
            regulatoryFramework: details.frameworksAssessed?.join(',')
        }, userId);
    }
    // Method to generate compliance dashboard data
    async generateComplianceDashboard() {
        const now = Date.now();
        const last30d = now - (30 * 24 * 60 * 60 * 1000);
        // Filter recent compliance-related events
        const recentEvents = this.auditChain.filter(e => e.timestamp > last30d && (e.category === 'COMPLIANCE' ||
            e.category === 'REGULATORY' ||
            e.eventType.includes('VERIFICATION') ||
            e.eventType.includes('DUE_DILIGENCE')));
        // Calculate verification metrics
        const verificationEvents = recentEvents.filter(e => e.eventType === 'VERIFICATION_COMPLETED');
        const successfulVerifications = verificationEvents.filter(e => e.details?.status === 'VERIFIED').length;
        const averageConfidence = verificationEvents.length > 0 ?
            verificationEvents.reduce((sum, e) => sum + (e.details?.confidence || 0), 0) / verificationEvents.length : 0;
        // Calculate due diligence metrics
        const ddEvents = recentEvents.filter(e => e.eventType.includes('DD_'));
        const completedProfiles = ddEvents.filter(e => e.details?.status === 'COMPLETED').length;
        const averageRiskScore = ddEvents.length > 0 ?
            ddEvents.reduce((sum, e) => sum + (e.details?.riskScore || 0), 0) / ddEvents.length : 0;
        // Calculate compliance rate
        const complianceAssessments = recentEvents.filter(e => e.eventType.includes('COMPLIANCE_ASSESSMENT'));
        const compliantAssessments = complianceAssessments.filter(e => e.details?.overallStatus === 'COMPLIANT').length;
        const complianceRate = complianceAssessments.length > 0 ?
            (compliantAssessments / complianceAssessments.length) * 100 : 0;
        // Calculate escalation rate
        const escalatedEvents = recentEvents.filter(e => e.details?.escalated || e.details?.escalationLevel > 0).length;
        const escalationRate = recentEvents.length > 0 ?
            (escalatedEvents / recentEvents.length) * 100 : 0;
        // Risk distribution
        const riskDistribution = new Map();
        ddEvents.forEach(e => {
            const riskRating = e.details?.riskRating || 'UNKNOWN';
            riskDistribution.set(riskRating, (riskDistribution.get(riskRating) || 0) + 1);
        });
        // Jurisdiction breakdown
        const jurisdictionBreakdown = new Map();
        recentEvents.forEach(e => {
            const jurisdiction = e.metadata?.jurisdiction || 'UNKNOWN';
            jurisdictionBreakdown.set(jurisdiction, (jurisdictionBreakdown.get(jurisdiction) || 0) + 1);
        });
        // Compliance frameworks
        const complianceFrameworks = new Map();
        recentEvents.forEach(e => {
            if (e.metadata?.regulatoryFramework) {
                const frameworks = e.metadata.regulatoryFramework.split(',');
                frameworks.forEach(framework => {
                    complianceFrameworks.set(framework, (complianceFrameworks.get(framework) || 0) + 1);
                });
            }
        });
        return {
            summary: {
                ...
                averageConfidence,
                totalDueDiligenceProfiles: ddEvents.length,
                completedProfiles,
                averageRiskScore,
                complianceRate,
                escalationRate
            },
            recentActivity: recentEvents.slice(0, 50), // Last 50 events
            riskDistribution,
