 ...
 * - Real-time WebSocket notifications for asset updates
 *
 * @version 1.0.0
...
public class AssetTraceabilityService {
    ...
    public Uni<AssetTrace> createAssetTrace(String assetId, String assetName, String assetType,
        ...
        return Uni.createFrom().item(() -> {
            ...
            AssetUpdateMessage wsMessage = new AssetUpdateMessage(traceId, assetId, AssetUpdateMessage.Action.CREATED);
            wsMessage.setAssetType(assetType);
            wsMessage.setCurrentOwner(owner);
            wsMessage.setValuation(valuation);
            wsMessage.setComplianceStatus(trace.getComplianceStatus());
            wsMessage.setDetails(Map.of(
                "assetName", assetName,
                "createdAt", Instant.now()
            ));
            webSocketMessageService.broadcastAssetUpdate(traceId, wsMessage);
    ...
    public Uni<OwnershipRecord> transferOwnership(String traceId, String fromOwner, String toOwner,
        ...
        return Uni.createFrom().item(() -> {
            ...
            // Broadcast WebSocket update
            AssetUpdateMessage wsMessage = new AssetUpdateMessage(traceId, trace.getAssetId(), AssetUpdateMessage.Action.TRANSFERRED);
            wsMessage.setAssetType(trace.getAssetType());
            wsMessage.setCurrentOwner(toOwner);
            wsMessage.setValuation(trace.getValuation());
            wsMessage.setComplianceStatus(trace.getComplianceStatus());
            wsMessage.setDetails(Map.of(
                "fromOwner", fromOwner,
                "toOwner", toOwner,
