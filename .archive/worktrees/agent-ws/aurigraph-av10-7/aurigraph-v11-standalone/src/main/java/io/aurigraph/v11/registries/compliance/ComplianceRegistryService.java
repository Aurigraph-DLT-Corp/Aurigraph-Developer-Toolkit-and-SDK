...
public class ComplianceRegistryService {
    ...
    public Uni<ComplianceRegistryEntry> addCertification(
        ...
        return Uni.createFrom().item(() -> {
            ...
            ComplianceAlertMessage alert = new ComplianceAlertMessage(
                entityId, certificationId, ComplianceAlertMessage.Severity.INFO, ComplianceAlertMessage.Action.ISSUED
            );
            alert.setCertificationType(certificationType);
            alert.setComplianceLevel(complianceLevel.name());
            alert.setMessage("New certification issued: " + certificationType);
            alert.setExpiryDate(expiryDate);
            alert.setDetails(Map.of(
                "issuingAuthority", issuingAuthority,
                "issuanceDate", issuanceDate,
                "status", entry.getCurrentStatus().name()
            ));
            webSocketMessageService.broadcastComplianceAlert(alert);
    ...
    public Uni<ComplianceRegistryEntry> renewCertification(String certificationId, Instant newExpiryDate) {
        return Uni.createFrom().item(() -> {
            ...
            // Broadcast WebSocket alert
            ComplianceAlertMessage alert = new ComplianceAlertMessage(
                entry.getEntityId(), certificationId, ComplianceAlertMessage.Severity.INFO, ComplianceAlertMessage.Action.RENEWED
            );
            alert.setCertificationType(entry.getCertificationType());
            alert.setComplianceLevel(entry.getComplianceLevel().name());
            alert.setMessage("Certification renewed: " + entry.getCertificationType());
            alert.setExpiryDate(newExpiryDate);
            webSocketMessageService.broadcastComplianceAlert(alert);
    ...
    public Uni<ComplianceRegistryEntry> revokeCertification(String certificationId) {
        return Uni.createFrom().item(() -> {
            ...
            // Broadcast WebSocket alert
            ComplianceAlertMessage alert = new ComplianceAlertMessage(
                entry.getEntityId(), certificationId, ComplianceAlertMessage.Severity.CRITICAL, ComplianceAlertMessage.Action.REVOKED
            );
            alert.setCertificationType(entry.getCertificationType());
            alert.setComplianceLevel(entry.getComplianceLevel().name());
            alert.setMessage("Certification revoked: " + entry.getCertificationType());
            webSocketMessageService.broadcastComplianceAlert(alert);
