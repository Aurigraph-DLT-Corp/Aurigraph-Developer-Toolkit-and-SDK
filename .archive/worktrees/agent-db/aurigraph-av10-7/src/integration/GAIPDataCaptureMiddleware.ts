...
import { GAIPIntegrationModule, GAIPDatapoint, GAIPAnalysis } from './GAIPIntegrationModule';
...
export class GAIPDataCaptureMiddleware {
    ...
    constructor(integrationModule: GAIPIntegrationModule) {
        this.logger = new Logger('GAIPMiddleware');
     ...
     */
    private async authenticate(req: GAIPRequest, res: Response, next: NextFunction): Promise<void> {
        const apiKey = req.headers['x-gaip-api-key'] as string;
        const agentId = req.headers['x-gaip-agent-id'] as string;
        ...
        req.gaipContext = {
            ...
            sessionId: req.headers['x-gaip-session-id'] as string || uuidv4(),
            authenticated: true
        };
     ...
     */
    private async startAnalysis(req: GAIPRequest, res: Response): Promise<void> {
        try {
            const { name, description, agents } = req.body;
            const analysisId = uuidv4();

            await this.integrationModule.startAnalysisCapture(
                analysisId,
                name,
                description,
                agents || [{ id: req.gaipContext?.agentId, type: 'GAIP', role: 'ANALYZER', version: '1.0' }]
            );
     ...
     */
    private async completeAnalysis(req: GAIPRequest, res: Response): Promise<void> {
        try {
            const { id } = req.params;
     ...
     * Get analysis status
     */
    private async getAnalysisStatus(req: GAIPRequest, res: Response): Promise<void> {
        try {
            const { id } = req.params;
     ...
     */
    private async captureDatapoint(req: GAIPRequest, res: Response): Promise<void> {
        try {
            const datapoint: GAIPDatapoint = {
                id: uuidv4(),
                timestamp: Date.now(),
     ...
     */
    private async captureBatch(req: GAIPRequest, res: Response): Promise<void> {
        try {
     ...
     * Query datapoints
     */
    private async queryDatapoints(req: GAIPRequest, res: Response): Promise<void> {
        try {
            const filter = {
                analysisId: req.query.analysisId as string,
     ...
     */
    private async verifyDatapoint(req: Request, res: Response): Promise<void> {
        try {
     ...
     */
    private async exportAnalysis(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const format = (req.query.format as 'JSON' | 'CSV' | 'BLOCKCHAIN') || 'JSON';
     ...
     */
    private handleWebSocketConnection(ws: any, req: Request): void {
    ...
    private async handleStreamStartAnalysis(ws: any, data: any): Promise<void> {
        const analysisId = uuidv4();
    ...
    private async handleStreamDatapoint(ws: any, data: any): Promise<void> {
        const datapoint: GAIPDatapoint = {
            id: uuidv4(),
            timestamp: Date.now(),
            ...data.datapoint,
    ...
    private async handleStreamCompleteAnalysis(ws: any, data: any): Promise<void> {
        const transactionId = await this.integrationModule.completeAnalysis(
            data.analysisId,
            data.results
        );
