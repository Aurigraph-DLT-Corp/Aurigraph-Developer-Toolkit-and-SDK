...
/**
 * JIRA Integration Fix v3 - Bug Fixed
...
console.log('🔧 JIRA Integration Fix v3');
console.log('===========================\n');
...
async function createTicket(ticket, issueTypes) {
    try {
        ...
        if (!selectedIssueType) {
            ...
            selectedIssueType = issueTypes.find(type => type.name.toLowerCase().includes('task'));
            if (!selectedIssueType) {
        ...
        console.log(`📝 Creating: ${ticket.summary.substring(0, 60)}...`);
        ...
        console.error(`❌ Failed to create '${ticket.summary.substring(0, 40)}...':`, error.response?.data?.errors || error.message);
        return null;
    }
}
