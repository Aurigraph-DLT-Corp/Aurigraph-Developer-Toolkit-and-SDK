...
class NeuralNetworkEngine extends events_1.EventEmitter {
    constructor(quantumCrypto, useGPU = true) {
        ...
        this.continualLearning = new Map();
    ...
    updateWeightsAdam(layer, learningRate) {
        ...
        if (!layer.gradients.weights['momentum']) {
            layer.gradients.weights.momentum = new Float32Array(layer.weights.length);
            layer.gradients.weights.velocity = new Float32Array(layer.weights.length);
        }
        const momentum = layer.gradients.weights.momentum;
        const velocity = layer.gradients.weights.velocity;
        for (let i = 0; i < layer.weights.length; i++) {
    ...
    updateWeightsRMSprop(layer, learningRate) {
        ...
        if (!layer.gradients.weights['rms']) {
            layer.gradients.weights.rms = new Float32Array(layer.weights.length);
        }
        const rms = layer.gradients.weights.rms;
        for (let i = 0; i < layer.weights.length; i++) {
            const gradient = layer.gradients.weights[i];
