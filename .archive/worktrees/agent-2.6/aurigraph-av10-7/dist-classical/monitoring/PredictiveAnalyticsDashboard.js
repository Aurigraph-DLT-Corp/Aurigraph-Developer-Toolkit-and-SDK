...
exports.PredictiveAnalyticsDashboard = void 0;
...
class PredictiveAnalyticsDashboard {
    ...
    constructor(analyticsEngine, metricsCollector) {
        this.logger = new Logger_1.Logger('PredictiveAnalyticsDashboard-AV10-26');
    ...
    setupEventListeners() {
        ...
        this.metricsCollector.on('drift_detected', (alert) => {
            this.handleDriftAlert(alert);
        });
        this.metricsCollector.on('feature_analytics_updated', (analytics) => {
    ...
    async start(port = 3040) {
        ...
        this.wss = new ws_1.WebSocketServer({ port: port + 1 });
    ...
    async updateModelMetrics(modelId) {
        try {
    ...
    createAlert(alert) {
        ...
        const existingAlert = this.driftAlerts.find(a => a.modelId === alert.modelId &&
        ...
        if (!existingAlert) {
            ...
            this.driftAlerts = this.driftAlerts.filter(a => Date.now() - a.detectedAt < 86400000);
    ...
    async handleConceptDrift(alert) {
    ...
    async handlePredictionDrift(alert) {
    ...
    handleGetModelDetails(ws, modelId) {
        const metrics = this.modelMetrics.get(modelId);
        if (metrics) {
            const response = {
                type: 'model_details',
...
<body>
    ...
    assessSystemHealth() {
        ...
        if (avgAccuracy > 0.9 && alertCount === 0 && errorRate < 0.01)
            return 'excellent';
        if (avgAccuracy > 0.85 && alertCount < 3 && errorRate < 0.05)
            return 'good';
        if (avgAccuracy > 0.75 && alertCount < 10 && errorRate < 0.1)
            return 'warning';
        return 'critical';
    }
    getAlertCounts() {
    ...
    updateHistoricalData() {
        ...
        for (const [modelId, metrics] of this.modelMetrics) {
            ...
            this.historicalData.set(modelId, history.filter(point => point.timestamp > sevenDaysAgo));
        }
    }
    inferModelType(modelId) {
        if (modelId.includes('asset'))
            return 'asset_valuation';
        if (modelId.includes('market'))
            return 'market_trend';
        if (modelId.includes('risk'))
            return 'risk_assessment';
        if (modelId.includes('performance'))
            return 'performance_forecast';
        if (modelId.includes('anomaly'))
    ...
    storeHistoricalDataPoint(modelId, metrics) {
        if (!this.historicalData.has(modelId)) {
            this.historicalData.set(modelId, []);
        }
        const history = this.historicalData.get(modelId);
    ...
    categorizeDriftSeverity(driftScore) {
        if (driftScore > 0.9)
            return 'critical';
        if (driftScore > 0.7)
            return 'high';
    ...
    async stop() {
        ...
        if (this.clientBroadcastInterval)
            clearInterval(this.clientBroadcastInterval);
...
exports.PredictiveAnalyticsDashboard = PredictiveAnalyticsDashboard;
//# sourceMappingURL=PredictiveAnalyticsDashboard.js.map