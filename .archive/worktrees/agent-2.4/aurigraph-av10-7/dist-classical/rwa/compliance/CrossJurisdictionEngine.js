...
exports.CrossJurisdictionEngine = void 0;
const events_1 = require("events");
class CrossJurisdictionEngine extends events_1.EventEmitter {
    jurisdictionRules = new Map();
    complianceAssessments = new Map();
    automatedReports = new Map();
    cryptoManager;
    ...
    async assessMultiJurisdictionCompliance(user, asset, transactionType) {
        ...
        for (const jurisdiction of applicableJurisdictions) {
            ...
            if (!result.compliant) {
                ...
                assessment.requiredActions.push(...result.failedRules.map(rule => `${jurisdiction}: Address ${rule}`));
            }
    ...
    determineApplicableJurisdictions(user, asset) {
    ...
    async assessJurisdictionCompliance(jurisdiction, user, asset, transactionType) {
    ...
    async evaluateComplianceRule(rule, user, asset, transactionType) {
    ...
    async aiEvaluateRule(rule, user, asset) {
    ...
    evaluateInvestmentRestriction(restriction, user, asset) {
    ...
    evaluateTaxCompliance(taxRule, user, asset, transactionType) {
    ...
    doesTaxRuleApply(taxRule, user, asset, transactionType) {
    ...
    async generateAutomatedReport(jurisdiction, reportType) {
        const rules = this.jurisdictionRules.get(jurisdiction);
        if (!rules) {
    ...
    async generateReportData(jurisdiction, reportType) {
        const baseData = {
    ...
    async simulateRegulatorySubmission(report) {
    ...
    async processAutomatedReporting() {
    ...
    async updateJurisdictionRules(jurisdiction, updates) {
    ...
    async getComplianceGaps(assessmentId) {
...
exports.CrossJurisdictionEngine = CrossJurisdictionEngine;
//# sourceMappingURL=CrossJurisdictionEngine.js.map