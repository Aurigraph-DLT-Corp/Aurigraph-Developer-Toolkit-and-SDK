...
exports.DigitalTwinDashboard = void 0;
...
class DigitalTwinDashboard {
    ...
    constructor(digitalTwinEngine, config) {
        this.logger = new Logger_1.Logger('DigitalTwinDashboard');
    ...
    setupDigitalTwinEngineListeners() {
        this.digitalTwinEngine.on('asset_created', (asset) => {
        ...
        this.digitalTwinEngine.on('predictions_updated', (data) => {
    ...
    setupRoutes() {
        ...
        this.app.get('/api/analytics/performance', (req, res) => {
            const timeRange = parseInt(req.query.timeRange) || 3600; // 1 hour
...
<body>
    ...
    async handlePredictionsUpdated(data) {
    ...
    calculatePredictiveScore(asset) {
        if (asset.predictedValues.length === 0)
            return 0;
        return asset.predictedValues.reduce((sum, p) => sum + p.confidence, 0) / asset.predictedValues.length * 100;
    }
    calculateMaintenanceScore(asset) {
        const now = Date.now();
        let score = 100;
    ...
    async createCustomDashboard(assetId, config) {
    ...
    generatePerformanceAnalytics(timeRange) {
        const now = Date.now();
        ...
        return {
            ...
            alerts: Array.from(this.activeAlerts.values()).filter(a => a.timestamp >= startTime && a.timestamp <= now)
        };
    }
    generatePredictionAnalytics(assetId) {
        if (assetId) {
            const asset = this.digitalTwinEngine.getAsset(assetId);
            if (asset) {
                return {
    ...
    async start(port = 3040) {
        ...
        this.wss.on('connection', (ws) => {
            this.clients.add(ws);
            this.performanceStats.peakConcurrentUsers = Math.max(this.performanceStats.peakConcurrentUsers, this.clients.size);
...
exports.DigitalTwinDashboard = DigitalTwinDashboard;
exports.default = DigitalTwinDashboard;
//# sourceMappingURL=DigitalTwinDashboard.js.map