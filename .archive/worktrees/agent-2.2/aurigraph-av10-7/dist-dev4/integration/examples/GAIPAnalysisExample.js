...
exports.GAIPAnalysisExample = void 0;
const axios_1 = __importDefault(require("axios"));
const Logger_1 = require("../../core/Logger");
class GAIPAnalysisExample {
    logger;
    apiUrl;
    apiKey;
    agents;
    dataSources;
    constructor(apiUrl = 'http://localhost:3005/api/v1/gaip', apiKey = 'demo-api-key') {
        this.logger = new Logger_1.Logger('GAIPExample');
        this.apiUrl = apiUrl;
        this.apiKey = apiKey;
    ...
    async startAnalysis() {
        ...
        const response = await axios_1.default.post(`${this.apiUrl}/analysis/start`, {
     ...
     */
    async collectInputData(analysisId) {
     ...
     */
    async processWithAgents(analysisId) {
     ...
     */
    async generatePredictions(analysisId) {
     ...
     */
    async makeDecisions(analysisId) {
     ...
     * Validate results
     */
    async validateResults(analysisId) {
    ...
    async completeAnalysis(analysisId) {
        ...
        const response = await axios_1.default.post(`${this.apiUrl}/analysis/${analysisId}/complete`, { results }, {
            headers: {
                'x-gaip-api-key': this.apiKey,
                'x-gaip-agent-id': 'orchestrator-001'
            }
     ...
     */
    async verifyOnBlockchain(analysisId) {
        this.logger.info('🔐 Verifying on blockchain');
        // Get report to see all recorded datapoints
        const reportResponse = await axios_1.default.get(`${this.apiUrl}/analysis/${analysisId}/report`, {
            headers: {
                'x-gaip-api-key': this.apiKey
            }
        });
        ...
        for (const datapoint of datapointsToVerify) {
            const verifyResponse = await axios_1.default.get(`${this.apiUrl}/verify/${datapoint.id}`);
     ...
     */
    async captureDatapoint(datapoint, agentId) {
        await axios_1.default.post(`${this.apiUrl}/datapoint`, datapoint, {
            headers: {
                'x-gaip-api-key': this.apiKey,
    ...
    async runSimpleTest() {
        ...
        const analysisId = response.data.analysisId;
        // Capture a datapoint
        await axios_1.default.post(`${this.apiUrl}/datapoint`, {
            analysisId,
            dataType: 'INPUT',
            category: 'TEST',
        ...
        // Complete analysis
        await axios_1.default.post(`${this.apiUrl}/analysis/${analysisId}/complete`, { results: { test: 'success' } }, {
            headers: {
                'x-gaip-api-key': this.apiKey,
                'x-gaip-agent-id': 'test-agent'
            }
...
exports.GAIPAnalysisExample = GAIPAnalysisExample;
...
//# sourceMappingURL=GAIPAnalysisExample.js.map