...
class MCPRouter {
    ...
    mcpInterface;
    auditManager;
    constructor(assetRegistry, auditManager, multiAssetManager) {
    ...
    setupRoutes() {
        ...
        this.router.use(async (req, res, next) => {
            try {
                ...
                if (!isValid) {
                    await this.auditManager.logEvent('MCP_AUTH_FAILED', 'ACCESS', 'HIGH', req.headers['x-request-id'] || 'unknown', 'MCP_REQUEST', 'AUTH_FAILED', {
                        clientId,
                        path: req.path,
                        method: req.method,
                        ip: req.ip
        ...
        this.router.get('/assets', async (req, res) => {
            try {
                ...
                const result = await this.mcpInterface.processRequest({
                    id: req.headers['x-request-id'] || require('crypto').randomUUID(),
                    method: 'assets.list',
                    params: req.query,
        ...
        this.router.get('/analytics/market', async (req, res) => {
            try {
                ...
                const result = await this.mcpInterface.processRequest({
                    ...
                    params: { timeframe: req.query.timeframe },
                    apiKey: req.headers['x-api-key'],
                    timestamp: Date.now(),
                    signature: req.headers['x-signature'],
                    version: '2.0',
        ...
        this.router.post('/batch', async (req, res) => {
            try {
                ...
                const responses = await Promise.all(requests.map((request) => this.mcpInterface.processRequest({
                    ...request,
                    apiKey: req.headers['x-api-key'],
    ...
    async validateAuthentication(apiKey, clientId, signature, requestBody) {
    ...
    async registerWebhook(clientId, url, events, secret) {
        ...
        await this.auditManager.logEvent('WEBHOOK_REGISTERED', 'ACCESS', 'MEDIUM', webhookId, 'WEBHOOK', 'REGISTER', {
            clientId,
            url,
            events,
            hasSecret: !!secret
    ...
    async unregisterWebhook(clientId, webhookId) {
        await this.auditManager.logEvent('WEBHOOK_UNREGISTERED', 'ACCESS', 'MEDIUM', webhookId, 'WEBHOOK', 'UNREGISTER', {
            clientId,
            webhookId
        }, { nodeId: 'mcp-router' });
    }
