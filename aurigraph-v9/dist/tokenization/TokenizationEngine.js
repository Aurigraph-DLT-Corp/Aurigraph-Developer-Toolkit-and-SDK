...
exports.TokenizationEngine = void 0;
const events_1 = require("events");
const Logger_1 = require("../utils/Logger");
const DigitalTwinManager_1 = require("./DigitalTwinManager");
const AssetRegistry_1 = require("./AssetRegistry");
const ComplianceEngine_1 = require("./ComplianceEngine");
const SmartContractEngine_1 = require("../contracts/SmartContractEngine");
const types_1 = require("./types");
class TokenizationEngine extends events_1.EventEmitter {
    logger;
    assetRegistry;
    complianceEngine;
    contractEngine;
    ...
    constructor() {
        super();
        this.logger = new Logger_1.Logger('TokenizationEngine');
        this.assetRegistry = new AssetRegistry_1.AssetRegistry();
        this.complianceEngine = new ComplianceEngine_1.ComplianceEngine();
        this.contractEngine = new SmartContractEngine_1.SmartContractEngine();
        this.digitalTwinManager = new DigitalTwinManager_1.DigitalTwinManager();
    }
    async initialize() {
    ...
    async tokenizeAsset(asset, tokenizationParams) {
        ...
        const tokenStructure = await this.generateTokenStructure(registeredAsset, tokenizationParams);
    ...
    async verifyAssetOwnership(asset) {
        this.logger.info(`Verifying ownership for asset: ${asset.assetId}`);
        const ownershipVerification = await this.complianceEngine.verifyOwnership(asset.owner, asset.documents || []);
    ...
    async generateTokenStructure(asset, params) {
        ...
        if (params.includePrimaryTokens) {
            structure.primaryTokens.push({
                ...
                tokenType: types_1.TokenType.PRIMARY,
        ...
        if (params.includeSecondaryTokens) {
            structure.secondaryTokens.push({
                ...
                tokenType: types_1.TokenType.SECONDARY,
        ...
        if (params.includeCompoundTokens) {
            structure.compoundTokens.push({
                ...
                tokenType: types_1.TokenType.COMPOUND,
    ...
    async deployTokenContracts(tokenStructure) {
    ...
    async mintTokens(contracts, tokenStructure) {
        this.logger.info('Minting tokens...');
        const tokens = [];
        for (const contract of contracts) {
            const mintResult = await contract.mint(contract.tokenMetadata.totalSupply, contract.tokenMetadata.initialOwner);
            tokens.push({
    ...
    async setupGovernance(tokens, governance) {
        ...
        for (const token of tokens) {
            if (token.tokenType === types_1.TokenType.SECONDARY || token.tokenType === types_1.TokenType.COMPOUND) {
                await this.contractEngine.linkGovernance(token.contractAddress, governanceContract.address);
            }
        }
    }
    async configureCompliance(tokens, jurisdiction) {
        ...
        for (const token of tokens) {
            if (token.transferable) {
                await this.contractEngine.setComplianceRules(token.contractAddress, complianceRules);
            }
        }
    }
    ...
    async updateAssetValue(assetId, newValue) {
        ...
        await this.assetRegistry.updateAsset(asset);
        await this.digitalTwinManager.updateDigitalTwinState(asset.digitalTwinId, { value: newValue });
...
exports.TokenizationEngine = TokenizationEngine;
//# sourceMappingURL=TokenizationEngine.js.map