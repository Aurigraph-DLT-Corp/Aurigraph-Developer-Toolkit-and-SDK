 ...
 * - PUT /versions/{versionNumber}/vvb-approve - VVB approval
...
public class SecondaryTokenVersionResource {
    ...
    public Uni<Response> activateVersion(
        ...
        return Uni.createFrom().item(() -> {
            ...
            return version;
                ...
                .flatMap(version -> versioningService.activateVersion(version.getId()))
                .map(version -> {
                    log.info("Version {} activated", versionNumber);
                    return Response.ok(SecondaryTokenVersionDTO.fromEntity(version)).build();
                })
                .onFailure().recoverWithItem(failure -> {
     ...
     * PUT /api/v12/secondary-tokens/{tokenId}/versions/{versionNumber}/vvb-submit
     *
     * @param tokenId Token ID
     * @param versionNumber Version number
     * @return Version in PENDING_VVB status
     */
    @PUT
    @Path("/{versionNumber}/vvb-submit")
    @Produces(MediaType.APPLICATION_JSON)
    public Uni<Response> submitForVVBApproval(
            @PathParam("tokenId") UUID tokenId,
            @PathParam("versionNumber") Integer versionNumber) {

        log.info("Submitting version {} for VVB approval", versionNumber);
        ...
        return Uni.createFrom().item(() -> findVersionByNumber(tokenId, versionNumber))
                .flatMap(version -> {
                    ...
                    return versioningService.submitForVVBApproval(version.getId());
                })
                .map(version -> {
                    if (version == null) {
                        return Response.status(Response.Status.NOT_FOUND)
                ...
                .onFailure().recoverWithItem(failure -> {
                    log.error("Failed to submit for VVB approval", failure);
                    return Response.status(Response.Status.BAD_REQUEST)
                            .entity(new ErrorResponse("Submission failed: " + failure.getMessage()))
                            .build();
                });
    }

    /**
     * VVB approval of version
     * PUT /api/v12/secondary-tokens/{tokenId}/versions/{versionNumber}/vvb-approve
     *
     * @param tokenId Token ID
     * @param versionNumber Version number
     * @param approver Approver identifier (header or body)
     * @return Activated version
     */
    @PUT
    @Path("/{versionNumber}/vvb-approve")
    @Produces(MediaType.APPLICATION_JSON)
    public Uni<Response> approveVersion(
        ...
        log.info("VVB approving version {} by {}", versionNumber, approver);
        ...
        return Uni.createFrom().item(() -> findVersionByNumber(tokenId, versionNumber))
                .flatMap(version -> {
                    ...
                    return versioningService.approveWithVVB(version.getId(), approver);
                })
                .map(version -> {
                    if (version == null) {
                        return Response.status(Response.Status.NOT_FOUND)
                ...
                .onFailure().recoverWithItem(failure -> {
                    log.error("Failed to approve version", failure);
                    return Response.status(Response.Status.BAD_REQUEST)
                            .entity(new ErrorResponse("Approval failed: " + failure.getMessage()))
                            .build();
                });
    }
