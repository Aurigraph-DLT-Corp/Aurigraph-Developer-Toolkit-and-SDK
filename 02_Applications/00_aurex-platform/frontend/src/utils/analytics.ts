...
declare global {
  interface Window {
    ...
    dataLayer: unknown[];
    fbq?: (command: string, event: string, properties?: Record<string, unknown>) => void;
    lintrk?: {
      (command: string, config: Record<string, unknown>): void;
      q?: unknown[];
    };
  }
}
...
// Lead tracking
export const trackLead = (leadType: string, properties: Record<string, unknown> = {}) => {
  trackEvent({
    event: 'generate_lead',
    properties: {
      lead_type: leadType,
...
export const trackTrialSignup = (planType: string) => {
  trackEvent({
    event: 'sign_up',
    properties: {
      method: 'trial',
...
export const trackVideoEngagement = (videoName: string, action: string, progress?: number) => {
  trackEvent({
...
const initFacebookPixel = () => {
  ...
  if (FACEBOOK_PIXEL_ID && typeof window !== 'undefined') {
    (function(f: Window, b: Document, e: string, v: string, n?: HTMLScriptElement, t?: HTMLScriptElement, s?: HTMLElement) {
      if (f.fbq) return;
      n = f.fbq = function() {
...
const initHotjar = () => {
  ...
  if (HOTJAR_ID && typeof window !== 'undefined') {
    (function(h: Window, o: Document, t: string, j: string, a?: HTMLScriptElement, r?: HTMLScriptElement) {
      h.hj = h.hj || function() {
...
const trackWebVitals = () => {
  import('web-vitals').then(({ onCLS, onFCP, onLCP, onTTFB }) => {
    onCLS((metric: { value: number }) => trackEvent({ event: 'web_vitals', properties: { metric_name: 'CLS', value: metric.value } }));
    onFCP((metric: { value: number }) => trackEvent({ event: 'web_vitals', properties: { metric_name: 'FCP', value: metric.value } }));
    onLCP((metric: { value: number }) => trackEvent({ event: 'web_vitals', properties: { metric_name: 'LCP', value: metric.value } }));
    onTTFB((metric: { value: number }) => trackEvent({ event: 'web_vitals', properties: { metric_name: 'TTFB', value: metric.value } }));
  }).catch(() => {
