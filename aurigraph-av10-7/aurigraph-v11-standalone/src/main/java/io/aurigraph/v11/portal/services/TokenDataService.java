 ...
 * - Call tokenManagementService.getTokenBalance(tokenId, address) for balance
 * queries
 */
@ApplicationScoped
public class TokenDataService {
    ...
    public Uni<TokenDTO> createToken(TokenCreateRequest request) {
        ...
        var tokenRequest = io.aurigraph.v11.tokens.TokenManagementService.RWATokenRequest.builder()
                ...
                .totalSupply(
                        request.value() != null ? BigDecimal.valueOf(request.value()) : BigDecimal.valueOf(1000000L))
                .maxSupply(request.value() != null ? BigDecimal.valueOf(request.value()) : BigDecimal.valueOf(1000000L))
                .assetType(request.assetType() != null
                        ? io.aurigraph.v11.contracts.models.AssetType.valueOf(request.assetType().toUpperCase())
                        : io.aurigraph.v11.contracts.models.AssetType.REAL_ESTATE)
                .owner("system") // Default owner
                .assetId("ASSET-" + UUID.randomUUID().toString().substring(0, 8))
                .assetValue(request.value() != null ? BigDecimal.valueOf(request.value()) : BigDecimal.ZERO)
                .assetCurrency("USD")
                .isMintable(true)
                .isBurnable(true)
                .kycRequired(false)
        ...
        return tokenManagementService.createRWAToken(tokenRequest)
                .onItem().transform(token -> {
                    return TokenDTO.builder()
                            ...
                            .type(token.getTokenType().toString().toLowerCase())
