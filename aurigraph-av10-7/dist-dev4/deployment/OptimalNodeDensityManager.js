...
exports.OptimalNodeDensityManager = void 0;
...
class OptimalNodeDensityManager extends events_1.EventEmitter {
    ...
    constructor(quantumCrypto, config) {
        super();
        this.logger = new Logger_1.Logger('AV10-32-OptimalNodeDensity');
    ...
    async calculateOptimalDensity() {
        ...
        const memoryConstrainedNodes = Math.floor((resources.availableMemoryGB * this.config.targetMemoryUtilization * 1024) / nodeReqs.memoryMB);
        const cpuConstrainedNodes = Math.floor((resources.cpuCores * this.config.targetCpuUtilization) / nodeReqs.cpuCores);
        const diskConstrainedNodes = Math.floor((resources.availableDiskGB * this.config.targetDiskUtilization * 1024) / nodeReqs.diskMB);
        const networkConstrainedNodes = Math.floor(resources.networkBandwidthMBps / nodeReqs.networkMBps);
        // Optimal node count is limited by the most constraining resource
        let optimalNodeCount = Math.min(memoryConstrainedNodes, cpuConstrainedNodes, diskConstrainedNodes, networkConstrainedNodes, this.config.maxNodesPerContainer);
    ...
    async optimizeNodeAllocation() {
        ...
        const underperformingNodes = nodes.filter(n => n.healthScore < 80 ||
    ...
    async getDiskSpace() {
        try {
            return new Promise((resolve, reject) => {
                (0, child_process_1.exec)('df -BG /', (error, stdout) => {
                    if (error) {
    ...
    async launchNodeProcess(node) {
        try {
            ...
            const nodeConfig = {
                ...
                consensusRole: (node.type === 'VALIDATOR' ? 'LEADER' : 'FOLLOWER'),
    ...
    async shutdownNodeProcess(node) {
        try {
        ...
        this.monitoringInterval = setInterval(async () => {
            try {
                ...
                if (this.config.scalingPolicy.enableAggressiveScaling || this.config.scalingPolicy.enablePredictiveScaling) {
                    ...
                    const avgResourceUtilization = (this.systemResources.memoryUtilization +
                        this.systemResources.cpuUtilization +
                        this.systemResources.diskUtilization) / 3;
    ...
    async stop() {
        ...
        const shutdownPromises = Array.from(this.nodes.keys()).map(nodeId => this.terminateNode(nodeId));
        await Promise.allSettled(shutdownPromises);
        this.logger.info('✅ Optimal Node Density Manager stopped');
        this.emit('stopped');
    }
    // Docker integration methods
    async buildDockerCommand(node, nodeConfig) {
    ...
    async executeDockerCommand(command, nodeId) {
        return new Promise((resolve) => {
    ...
    async waitForNodeHealth(node, timeoutMs = 60000) {
    ...
    async checkDockerContainerStatus(containerId) {
        return new Promise((resolve, reject) => {
            (0, child_process_1.exec)(`docker inspect --format='{{.State.Status}}' ${containerId}`, (error, stdout, stderr) => {
                if (error) {
    ...
    async checkNodeHealthEndpoint(node) {
        try {
    ...
    async shutdownDockerContainer(containerId) {
        return new Promise((resolve) => {
            // Graceful shutdown with 30 second timeout
            (0, child_process_1.exec)(`docker stop --time=30 ${containerId}`, (error, stdout, stderr) => {
                if (error) {
                    this.logger.error(`Failed to stop container ${containerId}: ${error.message}`);
                    // Force kill if graceful stop fails
                    (0, child_process_1.exec)(`docker kill ${containerId}`, (killError) => {
                        if (killError) {
    ...
    async removeDockerContainer(containerId) {
        return new Promise((resolve) => {
            (0, child_process_1.exec)(`docker rm ${containerId}`, (error, stdout, stderr) => {
                if (error) {
    ...
    async ensureDockerNetwork() {
        ...
        return new Promise((resolve) => {
            ...
            (0, child_process_1.exec)(`docker network ls --filter name=${networkName} --format "{{.Name}}"`, (error, stdout) => {
                if (error || !stdout.trim()) {
                    ...
                    (0, child_process_1.exec)(createNetworkCmd, (createError) => {
                        if (createError) {
    ...
    async forceScaling(action, nodeCount) {
        if (this.isScaling) {
    ...
    async createSpecificNode(type, customConfig) {
    ...
    async getDetailedNodeMetrics(nodeId) {
        const node = this.nodes.get(nodeId);
        if (!node) {
    ...
    async getDockerContainerStats(containerId) {
        return new Promise((resolve) => {
            (0, child_process_1.exec)(`docker stats ${containerId} --no-stream --format "table {{.CPUPerc}},{{.MemUsage}},{{.NetIO}},{{.BlockIO}}"`, (error, stdout) => {
                if (error) {
    ...
    calculateNodeResourceEfficiency(node) {
    ...
    calculateNodePerformanceRating(node) {
...
//# sourceMappingURL=OptimalNodeDensityManager.js.map