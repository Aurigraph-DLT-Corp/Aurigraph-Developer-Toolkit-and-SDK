...
exports.VizorMonitoringService = void 0;
...
class VizorMonitoringService extends events_1.EventEmitter {
    ...
    async recordConsensusMetrics(validatorId, metrics) {
    ...
    async recordChannelMetrics(channelId, metrics) {
        const timestamp = new Date();
    ...
    async recordQuantumMetrics(metrics) {
        const timestamp = new Date();
    ...
    async generateReport(dashboardId, timeRange) {
    ...
    generateWidgetSummary(metrics, type) {
        if (metrics.length === 0)
            return null;
