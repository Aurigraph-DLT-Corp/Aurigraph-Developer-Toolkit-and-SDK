...
print_help() {
    ...
    echo -e "  ${BLUE}--v11-port PORT${NC}     - Set V11 port (default: 9003)"
    ...
    echo -e "  ⚡ V11 API:    ${GREEN}http://localhost:9003${NC}"
...
wait_for_services() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        if curl -s "http://localhost:${V10_PORT:-4004}/health" >/dev/null 2>&1 && \
           curl -s "http://localhost:${V11_PORT:-9003}/api/v11/health" >/dev/null 2>&1 && \
...
case "$COMMAND" in
    "start")
        ...
        echo -e "${BLUE}🔧 Ports: V10=${V10_PORT:-4004}, V11=${V11_PORT:-9003}, Mgmt=${MANAGEMENT_PORT:-3040}${NC}"
        echo
        ...
        if wait_for_services; then
            ...
            echo -e "  ⚡ V11 Platform:         ${GREEN}http://localhost:${V11_PORT:-9003}${NC}"
            echo
    ...
    "status")
        ...
        if curl -s "http://localhost:${V11_PORT:-9003}/api/v11/health" >/dev/null 2>&1; then
